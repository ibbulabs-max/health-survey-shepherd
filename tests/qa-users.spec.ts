...
test.describe('QA User Creation Flow', () => {
  ...
  test('Create QA Users and Change Passwords', async ({ page }) => {
    ...
    await page.fill('input[placeholder="e.g. sup-placeholder"]', process.env.QA_SUP_USER || 'sup-placeholder');
    ...
    // --- 3. CREATE CHW ---
    await page.getByRole('button', { name: /Add User/i }).click();
    await page.fill('input[placeholder="e.g. sup-placeholder"]', process.env.QA_CHW_USER || 'chw-placeholder');
