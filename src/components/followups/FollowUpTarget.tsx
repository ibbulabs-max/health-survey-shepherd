...
export function FollowUpTarget({ completedTodayCount }: FollowUpTargetProps) {
  ...
  const progress =
    dailyTarget > 0 ? Math.min(100, Math.round((completedTodayCount / dailyTarget) * 100)) : 100;
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between">
            ...
            {canEdit && (
              <Button
                variant="ghost"
                size="icon"
                className="h-6 w-6 text-muted-foreground hover:text-foreground touch-target"
          ...
          <div className="mt-1 flex items-center justify-between text-xs">
            <span className="text-muted-foreground">
              <strong className="text-foreground">{completedTodayCount}</strong> of {dailyTarget}{" "}
              completed
            </span>
            {remaining === 0 ? (
