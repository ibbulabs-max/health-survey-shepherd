...
export function NotificationBell() {
  ...
  return (
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DropdownMenuContent align="end" className="w-80 sm:w-96 p-0 max-h-[80vh] overflow-hidden flex flex-col rounded-xl border-border/50 shadow-xl bg-background/95 backdrop-blur-xl">
        <div className="flex items-center justify-between p-4 pb-2">
          <DropdownMenuLabel className="p-0 font-display font-semibold text-base">Notifications</DropdownMenuLabel>
          {unreadCount > 0 && (
            <Button variant="ghost" size="sm" onClick={markAllAsRead} className="h-auto p-0 text-xs font-medium text-primary hover:text-primary/80 hover:bg-transparent">
              Mark all read
            </Button>
          )}
        </div>
            ...
            notifications.map((n) => (
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  <span className={`font-semibold ${!n.is_read ? "text-foreground" : "text-muted-foreground"}`}>
                    {n.title}
                  </span>
                ...
                {n.metadata?.link && (
                  <Link to={n.metadata.link} className="text-[11px] font-medium text-primary hover:underline mt-1 w-fit">
                    View details
                  </Link>
                )}
              </div>
