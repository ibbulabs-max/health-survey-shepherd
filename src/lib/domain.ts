...
export function calculateRisk(input: {
  ...
  if (conditions.length >= riskConfig.multipleConditionsThreshold) {
    escalate(level === "low" ? "moderate" : "high");
    reasons.push(`Multiple known conditions: ${conditions.join(", ")}`);
  } else if (conditions.length === 1) {
    escalate("moderate");
    reasons.push(`Known condition: ${conditions[0]}`);
  }
...
export const toStringArray = (value: unknown): string[] => {
  ...
  if (typeof value === "string")
    return value
      .split(/[,;/|]/)
      .map((s) => s.trim())
      .filter((s) => s && !/^(no|none|nil|na|n\/a)$/i.test(s));
  return [];
};
...
export function buildMemberView(
  ...
  const conditions = assessment
    ? toStringArray(assessment.known_history)
    : toStringArray(data["known_history"]);
...
export const followUpStatus = (
  ...
  const s = (status ?? "due").toLowerCase() as FollowUpStatus;
  if (s === "completed" || s === "skipped" || s === "rescheduled") return s;
