...
function TeamPage() {
  ...
  const supervisedCswIds = isSuperAdmin
    ...
    : teamMemberships.filter(t => t.supervisor_id === user?.id && t.status === 'active').map(t => t.csw_id);

  const teamMembers = (queryUsers.data ?? []).filter(u => supervisedCswIds.includes(u.profile.id));
  ...
  return (
    <div className="space-y-6 pb-12">
      <PageHeader
        ...
        subtitle={isSuperAdmin ? "Monitor all assigned Community Health Workers" : "Monitor and manage your Community Health Workers"}
      />
      
      {teamMembers.length === 0 ? (
        <EmptyState
          title="No CHWs Assigned"
          description={isSuperAdmin ? "No CHWs have been assigned to supervisors yet." : "You do not have any CHWs assigned to you."}
        />
      ) : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {teamMembers.map((chw) => {
            // Compute stats from dataset for this CHW
            const chwHouses = (dataset?.houses ?? []).filter(h => (h.house.data as Record<string, any>)?.['uploaded_by'] === chw.profile.id);
            const chwMembers = (dataset?.members ?? []).filter(m => m.houseUuid && chwHouses.some(h => h.house.id === m.houseUuid));
            const chwAssessments = (dataset?.members ?? []).filter(m => m.screenedAt && m.assessment?.assessed_by === chw.profile.id);
            ...
            const supervisorName = isSuperAdmin && membership ? (queryUsers.data ?? []).find(u => u.profile.id === membership.supervisor_id)?.profile.full_name : null;

            return (
              <div key={chw.profile.id} className="card-surface flex flex-col p-4 rounded-2xl shadow-xs border border-border/70 hover:border-primary/30 transition-colors">
                <div className="flex items-center gap-3 border-b border-border/50 pb-3 mb-3">
                  <div className="size-10 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                    <User className="size-5" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-base text-foreground truncate">{chw.profile.full_name || chw.profile.username}</h3>
                    <p className="text-xs text-muted-foreground truncate">
                      {isSuperAdmin && supervisorName ? `Sup: ${supervisorName}` : "Community Health Worker"}
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  ...
                  <Stat label="High Risk" value={highRiskCount} icon={ShieldAlert} valueClass={highRiskCount > 0 ? "text-destructive" : ""} />
                </div>
...
function Stat({ label, value, icon: Icon, valueClass }: { label: string, value: number, icon: any, valueClass?: string }) {
  return (
