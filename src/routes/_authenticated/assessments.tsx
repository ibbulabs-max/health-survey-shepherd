...
function AssessmentsIndexPage() {
  ...
  const completed = filteredMembers.filter(m => m.screenedAt).sort((a, b) => new Date(b.screenedAt!).getTime() - new Date(a.screenedAt!).getTime());
    ...
    <div className="space-y-6 pb-24">
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-display font-semibold text-base text-foreground">Pending Screenings ({pending.length})</h2>
        </div>
        ...
        {pending.length === 0 ? (
          <div className="card-surface p-6 rounded-2xl border border-border/70 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">No pending assessments for eligible members.</p>
          </div>
        ) : (
            ...
            {pending.slice(0, 20).map(member => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted-foreground font-mono">{member.memberId} • Age {member.age}</p>
                </div>
                <div className="flex items-center gap-2">
      ...
      <div className="space-y-4 pt-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-display font-semibold text-base text-foreground">Recent Assessments ({completed.length})</h2>
        </div>
        
        {completed.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-4">No completed assessments yet.</p>
        ) : (
          <div className="grid gap-2">
            {completed.slice(0, 10).map(member => (
              ...
              >
                <div>
                  <p className="font-bold text-sm text-foreground">{member.name}</p>
                  <p className="text-xs text-muted-foreground">Screened {new Date(member.screenedAt!).toLocaleDateString()}</p>
                </div>
                <div className="flex items-center gap-2">
                  <RiskBadge level={member.risk} />
